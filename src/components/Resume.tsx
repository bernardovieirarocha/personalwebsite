      ...
      <ol className="mt-6 space-y-4">
        {Work.map((role, roleIndex) => (
          <li key={roleIndex} className="flex gap-4">
            <div
              className={`relative mt-1 flex h-10 w-10 flex-none items-center justify-center rounded-full shadow-md shadow-zinc-800/5 ring-1 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:ring-0 ${role.company === 'CEFET-MG' ? 'bg-white dark:bg-white' : 'dark:bg-zinc-800'
                }`}
            >
              <Image
                src={role.logo}
